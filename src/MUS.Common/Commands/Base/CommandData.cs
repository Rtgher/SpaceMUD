...
namespace MUS.Common.Commands.Base
{
    public class CommandData
    {
        ...
        private static ILog Log = DependencyContainer.Provider.GetService(typeof(ILog)) as ILog;
            ...
            foreach (var memberInfo in properties)
            {
                ...
                if (data.UnspecifiedArguments.Count > 0)
                {
                    if (memberInfo.GetValue(data) == null)
                    {
                        ...
                        data.UnspecifiedArguments.Remove(((List<string>)data.UnspecifiedArguments).First());
